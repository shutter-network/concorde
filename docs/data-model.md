...
## Password Auth (`concorde_password_auth`)
...
| Field | Notes |
| --- | --- |
| `user_id` | the **primary key**, so a User holds one password. A foreign key onto `concorde_users.users.id`, and no cascade, because nothing removes a User |
...
| --- | --- |
| `id` | opaque |
| `user_id` | a foreign key onto `concorde_users.users.id`, **cross-schema now** where it used to be within one part, which is the second half of what the split cost (ADR-0052). It carries `on delete cascade` for a delete that cannot happen, so that the day a User can be removed the credentials go with them |
...
## Nostr Auth (`concorde_nostr_auth`)
...
| `user_id` | an **ordinary column**, indexed, and nothing here is unique per User. A foreign key onto `concorde_users.users.id`, with no cascade, because nothing removes a User |
...
## Nostr Channel (`concorde_nostr_channel`)
...
| Field | Notes |
| --- | --- |
| `user_id` | the **primary key**, so one User holds at most one Nostr key — otherwise there would be no answer to which one the agent writes back to. A foreign key onto `concorde_users.users.id`, and the only enforcement that this names a real User: the refusal is PostgreSQL's `23503` caught, with no lookup in front of the write. No cascade, because nothing removes a User |
...
| `user_id` | who the wrap is addressed to, and the column an Operator filters by. A foreign key onto `concorde_users.users.id`, like `pubkeys.user_id`. **Not** the recipient's Nostr public key: that is inside the wrap, where the Relay cannot read it either |
| `message_id` | the Message in the log this wrap carries, so an Operator can read what was not delivered. Deliberately **not** a foreign key onto `concorde_messenger.messages.id`: the wrap is opaque, so this is the only route from a stuck row back to the words, and a plain column answers it with one join |
...
- **A `typ` allowlist or reserved prefix.** Nothing is reserved and `concorde-decision+jws` is not special-cased; `typ` is the agent's own signed claim (ADR-0042).
