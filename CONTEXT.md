...
**Signing identity**:
The Ed25519 keypair a shared agent's **commitments** are checked against. The public half is what a verifier of a Decision uses; the Operator holds the private half **in trust**, which is the same trust the Operator already holds, applied to one more asset. Never enters the Agent Instance. **It is no longer the only keypair**: it once was, and this entry said "one shared agent, one keypair, and no second name for it", which the Nostr identity retired. Its audience is a third party who never touches the Gateway, and copying it forges commitments.
