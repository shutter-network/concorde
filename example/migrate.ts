 ...
 * effect of opening the Store, so a deploy can run this against the new schema *before*
 ...
 * same call — and to nothing else, because `store.migrate` is the only place the
 * per-part tracking tables can be checked for collisions.
 */

import { coreMigrations, defaultLogger, openStore } from "shared-agent-framework";
