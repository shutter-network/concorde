...
1. **Agent Implementation** — `pi` primarily, `openclaw` as the alternative, driven by a Runtime. In every example under [`examples/`](../examples/) it runs inside a container.
...
Every part a deployment is assembled from is a **Component**, and the eleven the framework ships are listed below. The first ten are in the order a deployment building them all starts them, and they stop in the reverse of it. Five of those have nothing to run and say so with a `start` and a `stop` that do nothing, which is what buys them a key and a position before the day they need one ([ADR-0037](./adr/0037-the-gateway-is-a-record-of-components.md)). The eleventh, the Nostr Channel, is in no order because it never stands beside the tenth: one Channel per Messenger is refused at registration, so a deployment runs it **instead of** the HTTP Channel and in that Channel's position, which is what `examples/03_nostr` does.
...
| Agent server | `agentServer` | framework, address from the Operator | — | reachable only by the Agent Implementation; a bare `Fastify()` wrapped in a `serverComponent` that holds its bind address until `start`, and published to nobody in every example, which is what keeps it reachable from the agent's network alone |
...
- Whether Users, the Messenger and the Scheduler stay parts of one deployable or become peer services later (ADR-0020). The foreign keys onto `concorde_users.users.id` — the Messenger's and the Nostr Channel's — are an argument for one deployable (ADR-0036, ADR-0049).
