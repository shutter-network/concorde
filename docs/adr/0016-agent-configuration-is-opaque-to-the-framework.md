...
The framework models nothing about what the agent *is*. Instructions, model choice, and tools are supplied by the deployment in whatever form its Agent Implementation expects, and the Runtime passes them through without interpreting them. All the framework knows when starting a Run is which Session to target and which Prompt to deliver.
...
- Swapping Agent Implementation means rewriting the agent's configuration. Signal Handlers, Users, Messages, and the Gateway's own behaviour carry over; nothing about the agent itself does.
...
- The Runtime's contract is narrow: start a Run against a Session with a Prompt, collect its output, report completion or failure.
