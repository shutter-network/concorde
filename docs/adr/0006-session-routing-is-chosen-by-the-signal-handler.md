...
- A Runtime must map "fresh" and "named" onto its Agent Implementation. `pi` supports this directly through `--session`, fork and clone; OpenClaw's `sessionKey` is comparable. Only `pi` supports branching.
