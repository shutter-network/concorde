...
**Public server**:
The HTTP server exposed outside the Gateway. Named for its exposure rather than its audience, because some of its routes serve no authenticated User. Users reach the User Manager and the HTTP Channel through it, and reach nothing else. A Channel that is not HTTP does not appear on it at all: the Nostr Channel's inbound edge is a Relay subscription, not a route.
...
Owned by the User Manager, not the Signal Worker, not the Messenger and not a Channel.
