...
**Token**:
What a User presents on every request after trading a password for one. Gateway-issued, always expiring, and revocable. **Password Auth's**, and not the framework's: it was the only request credential here until a second Auth arrived that issues nothing and takes a fresh signature on every request instead ([ADR-0053](./docs/adr/0053-nostr-auth-verifies-nip-98-per-request.md)). There is no umbrella term for "the thing a request presents", deliberately, that being a general word rather than one of this framework's. See [ADR-0030](./docs/adr/0030-passwords-are-traded-for-bearer-tokens.md) and [ADR-0052](./docs/adr/0052-authentication-is-a-component-again-and-the-public-server-aggregates.md).
...
**Authenticator**:
Rejected twice and now the name of a thing, under one syllable less. It was named in [ADR-0014](./docs/adr/0014-users-are-opaque-ids-and-authentication-is-pluggable.md) as the replaceable part that verifies a credential, and deleted by [ADR-0030](./docs/adr/0030-passwords-are-traded-for-bearer-tokens.md) on two arguments. One still holds and is why the population exists at all: **verification is the wrong seam**, since an implementation of one has to answer where the credential lives. The other assumed a deployment had one login, and plural Channels made login plural, so the seam turned out to be **who owns the secret**. See the **Auth** entry above, and [ADR-0052](./docs/adr/0052-authentication-is-a-component-again-and-the-public-server-aggregates.md). The word is kept here because the two rejections are the argument for the shape the thing ended up with, and because a glossary that had rejected *auth provider* by name adopting *Auth* is worth explaining rather than leaving as an apparent oversight: what that list refuses is a borrowed vendor phrase for the whole concept, not the morpheme.
