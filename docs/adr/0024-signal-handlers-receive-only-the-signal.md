...
- **A Prompt is `{ session, text }`**, where `session` names an existing Session and `null` requests a fresh one, and **the framework passes the name through untouched**. This bullet originally said the framework validated it at this seam against a transcription of `pi`'s session-id grammar, so that an invalid name failed at the handler rather than mid-Run, and it named `user_<id>` as the convention that grammar implies. Both are gone, and nothing replaced them: what makes a name acceptable is the Agent Implementation's to say ([ADR-0016](./0016-agent-configuration-is-opaque-to-the-framework.md)), and it says so itself. `pi` validates `--session-id` and exits 1 with its own message, which the Runtime puts in the failed Run's `error` beside the offending name in the Run's `session` — a diagnostic that cannot drift from the Agent Implementation the way a copy of its grammar can, and one a second Agent Implementation with different rules gets for free. What it costs is the pre-flight check over a whole fan-out: an invalid name now fails its own Run and the Prompts around it still run, which is what every other kind of failed Run already does ([ADR-0017](./0017-failed-runs-are-not-retried.md)). The framework names no Session convention at all — per-user Sessions are one of [ADR-0006](./0006-session-routing-is-chosen-by-the-signal-handler.md)'s topologies rather than the recommended one.
