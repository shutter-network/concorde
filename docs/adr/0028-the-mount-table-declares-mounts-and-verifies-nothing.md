...
hostRoot   { gatewayPath, hostPath }?                    absent means the Gateway is on the host

resolve    apply hostRoot (one pair), refuse an entry outside the root
...
`hostRoot` states it once, in the place where it is true, as **one pair and not a map**: `gatewayPath` is where the shared tree sits inside the Gateway's own container, and `hostPath` is where the daemon finds that same tree on the host. Absent, the mapping is identity and every entry's `gatewayPath` is its own source. Present, it is exhaustive — an entry equal to the root resolves to `hostPath` whole, one below it resolves to `hostPath` with the remainder appended, and an entry falling **outside** the root is **refused at resolution**, before anything is spawned, with a message naming the entry's path and the declared root. A containerised Gateway that declared a root too narrow for one of its mounts fails as a pure function rather than as an empty directory discovered weeks later. A trailing slash on the root's `gatewayPath` is the same directory and means nothing, which is the one tolerance the string matching grants.

The single pair replaced a map of `gatewayPath` prefixes matched longest-first, whose own rationale — "one fact about the deployment" — argued against it, since a record with three prefixes is three facts. Two costs come with the subtraction, and are recorded here rather than defended against. A Gateway whose shared tree spans **more than one host mount** becomes inexpressible through translation: one pair names one correspondence, and two separate host mounts a prefix map once bridged have no single `hostRoot` that covers both. Its escape is to declare **no** `hostRoot` and write daemon-namespace paths straight into each `gatewayPath`, which works because translation is the only thing the framework ever does with `gatewayPath` — at the price of entries the Gateway process itself cannot `ls`, since those strings no longer name anything in its own filesystem. The second cost is that the sentence which said named volumes fit as **exact-match entries** is **struck rather than caveated**. It never held: every mount is emitted as `type=bind`, whose source the daemon resolves as a host path and not as a volume name, so an exact key naming a volume was already a `type=bind` source pointing at a name that is not where the volume's data lives. Under `--mount type=bind` a volume is expressed the same as any other directory — the host path its `_data` occupies — and there is nothing special left for an exact match to preserve.

Discovering `hostRoot` automatically was considered and deferred, not rejected. Two mechanisms work: `/proc/self/mountinfo`, whose fourth field is the mount's root within its source filesystem, and `docker inspect` on the Gateway's own container, which is authoritative. The constraint recorded here for whoever picks this up: it must be the **exact** mechanism. The earlier argument that a heuristic was affordable rested on a round trip catching a bad guess, and there is no longer one. `mountinfo` returns a confident wrong path whenever the directory sits on a separate filesystem, a btrfs subvolume, or a Docker Desktop VM; `docker inspect` fails loudly or not at all.
...
Five things are still refused, and each is decidable from the value alone: a **missing image**, a **relative `agentPath`**, an **entry outside the `hostRoot`**, a **`.` or `..` segment** in any path the framework resolves — `agentPath`, `gatewayPath`, or the `hostRoot`'s `gatewayPath`, never `hostPath`, which is handed to the daemon unread — and a **duplicate `agentPath`**. The last two are string checks that need no I/O, which is the criterion the whole list is built on. A dot segment is what makes the prefix matching that decides "outside the root" unsound in the first place: `..` string-prefixes a root it resolves outside of, so an entry could read as covered while mounting anywhere, and `.` at the boundary reads as not covered when it is. The framework refuses it, naming the path, rather than normalizing on the Operator's behalf. A duplicate `agentPath` mounts two sources at one target, which the daemon refuses at the first Run — and under [ADR-0017](./0017-failed-runs-are-not-retried.md) that is a permanently dead Signal, not a startup error — so it moves here from the daemon's side of the line the moment it is seen to need no I/O. Three that look like they belong are not on it. A **mount source that does not exist** is the daemon's refusal at the first Run, above. An **image-internal symlink aliasing two `agentPath`s onto one target** stays the daemon's business too: the value never names what the image's own filesystem links where, so it is undecidable from here in a way a duplicate written out plainly is not. A **missing model** is now a line in a file the framework is not allowed to read ([ADR-0025](./0025-the-pi-adapter-spawns-one-confined-process-per-run.md)), so a deployment with no usable model constructs, starts, and fails its first Run permanently.
...
- **`gatewayPathFor` is deleted.** It existed so that one path could be recovered in the other direction, the Gateway-side path of a Session's transcript, and its only caller was the debug line ADR-0025 has now dropped along with the Session root. An exported reverse lookup with no caller is a claim that nothing tests. It would also not have stayed honest: `hostRoot` makes three names per mount, so an honest lookup is three-way, container path to Gateway path to host path, and a two-way one quietly picks one of the two answers on exactly the deployments `hostRoot` exists for.
