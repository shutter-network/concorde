 ...
 * Messenger's `messages.user_id`, the Nostr Channel's `pubkeys.user_id` and `outbox.user_id`,
 * Password Auth's `passwords.user_id` and `tokens.user_id`, and Nostr Auth's `grants.user_id`. A
 * configuration listing any of those `/schema` subpaths without this one generates a constraint
 * onto a table it never creates. `schemas.test.ts` pushes every part's schema together, which is
 ...
 * Password Auth, so a credential of any kind reaching this module again is the thing to
 * refuse in review: this component keeps identity, and the seam is who owns the secret.
 *
 * Nothing removes a User: no delete route, no deactivation, and no column holding one. Do not add a
 * `deactivated_at` for a capability no deployment has asked for. It would put a state and two
 * authentication branches into the code.
 */
