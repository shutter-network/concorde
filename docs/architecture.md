...
| Signatures | `signatures` | framework | public: verify a Signed Statement, fetch the public key — agent: sign anything | holds the signing identity and **no tables at all**, the only Component with none. `signingKey` is a `crypto.KeyObject` the Operator loads however they like; `signingAlg` is derived from it unless the key is RSA, which is ambiguous. Stores nothing, so signing is unrecorded beyond a log line (ADR-0041, ADR-0042) |
