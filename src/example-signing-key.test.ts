/**
 * `02_decisions`' committed signing key loads as committed.
 *
 * `examples/02_decisions/` ships a throwaway PKCS8 private key so that `docker compose up` comes
 * up from a fresh clone with no manual signing-key step, and the key's whole safety is that it is
 * a decoy marked worthless at every point of contact: its filename, `compose.yml`, `main.ts` and
 * the README (ADR-0041). None of that marking may go *inside* the PEM in a way that stops it
 ...
 * under `src/` rather than beside the key because the framework's own test runner
 * (`npm run check`) is what must execute it, and the file is read by a relative path from
 * `import.meta.url`, which resolves to the checkout's copy whether the marking stays beside the
 * key or ever moves into it.
 */
